...
function applyLiveSettings() {
    ...
    $.ajax({
        ...
        success: function(rdata) {
            ...
            setTimeout(function() { if (liveLoopActive) $("#live-status").text("Live - MJPEG stream"); }, 3000);
            // Refresh the img src to pick up the restarted stream
            refreshStreamSrc();
        },
        error: function(xhr) {
...
function startLive(retryCount) {
    ...
    $.ajax({
        ...
        success: function(rdata) {
            ...
            $("#live-status").text("Live - MJPEG stream");
            refreshStreamSrc();
        },
        error: function(xhr) {
...
function stopLive() {
    ...
    // Clear the stream src first to disconnect the client
    var target = document.getElementById("latest-image") || document.getElementById("loop-image");
    ...
    $.ajax({
        ...
        success: function(data) {
            if (data.running) {
                ...
                $("#live-status").text("Live - MJPEG stream (" + data.clients + " viewers)");
                refreshStreamSrc();
            }
        },
    });
